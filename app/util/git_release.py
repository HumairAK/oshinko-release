...
def main():
    ...
    if conf:
        ...
        tag_name, name, body, draft, prerelease, target_commitish, assets = \
            conf_yaml['tag_name'], conf_yaml['name'], conf_yaml['body'], conf_yaml['draft'], \
            conf_yaml['prerelease'], conf_yaml['target_commitish'], conf_yaml['assets']

        r = requests.get('{}/repos/{}/{}/releases/tags/{}'.format(API_ENDPOINT, user,
                                                                  repo_name, conf_yaml['tag_name']))
        if r.status_code == 200:
            error_msg = 'Repo release tag already exists.'
            log.error(error_msg)
